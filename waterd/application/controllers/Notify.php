...
class NotifyController extends Controller
{
    ...
    public function callbackAction()
    {
        ...
        if($result['direction'] == 'in'){
            ...
            $recharge_log = Db::name('recharge')->where(['order_no' => $result['hash']])->find();

            /** 如果有记录了则直接返回结果 */
            if (!empty($recharge_log))
            {
            ...
            $recharge['order_no'] = $result['hash'];
            $recharge['count'] = $result['num'];
            $recharge['create_time'] = substr($result['updatetime'],0,-3);
            $recharge['address'] = $address;
            $recharge['status'] = 1;
            if(strpos($result['coin_type'], 'DYX') !== false)
            {
                $recharge['coinType'] = 'DYX';
                $currency_id = 2;

            }else if(strpos($result['coin_type'], 'USDT') !== false)
            {
            ...
            if (empty($uid))
            {
                logs('未找到充值用户'.$address, '', 'notifyerror');
                echo json_encode(['code' => 0]);die();
            }
            $recharge['uid'] = $uid;
            ...
            // 获取提现记录
            $cash_log = Db::name('withdraw')->where(['order_no' => $order_no])->find();
            if(!empty($cash_log))
            {
                ...
                if ($cash_log['state'] == 2)
                {
                    ...
                    $res = Db::name('withdraw')->where(['order_no' => $order_no])->update($cash_up);
                    if (empty($res))
                    {
                        // 暂不支持币种
                    ...
                    try {
                        ...
                        if ($cash_log['service'] > 0)
                        {
                            // 增加平台收益
                            $wallet_model->addMoney(1, $cash_log['currency_id'], $cash_log['service'], $order_no, '提现手续费收益');
                            // 增加收益记录
                            $profit_model->addProfit($cash_log['id'], $cash_log['service'], $cash_log['currency_id'], 3);
                        }
