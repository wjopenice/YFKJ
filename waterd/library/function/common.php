...
function compareNiuNum($a, $b)
{
    ...
    if ($a == $b) {
        ...
        $aval = getNiuNiuNumber($a);
        $bval = getNiuNiuNumber($b);
        /** 判断点数 */
        if ($aval == $bval){
             $res = $a > $b ? $a : $b; // 相同点数比较红包大小
        } else if ($aval == 0 || $bval == 0){
             $res = $aval == 0 ? $a : $b; // 不同点数但是有0的时候返回值为0的
        } else if ($aval > $bval) {
            $res = $a; // 不相等且不等于0取大的
        } else {
            $res = $b;  // 不相等且不等于0取大的
