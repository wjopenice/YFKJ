...
class activityModel{
    ...
    public function get_sysuser_discount($username){
        $user = $this->db->field("id")
            ...
            ->find();
        $discount = $this->db->field("id,discount_num,lock_exchange_price,lock85_price,unlock_price,success_price,system_price,income,status,user_minimum_amount")
    ...
    public function get_news_user_order($username){
        ...
        foreach($result as $k=>$v){
            if($v['username'] == $username || $v['id'] == $username){
                $data['order'] = $k+1;
...
ON  FIND_IN_SET(b.id, a.users) GROUP BY guid ORDER BY money_total DESC";
    ...
    public function get_youinvutation($username,$disid,$price,$leavel){
        $user = $this->db->field("*")
            ->table("y_user")
            ->where("username = '{$username}'")
            ->find();
        ...
        $sql = "";
        if($leavel == 1){
            $sql = "select id,tel,token_available_balance,create_time from y_user where floor_id={$user_id}";
        }else if($leavel == 2){
            $sql = "select id,tel,token_available_balance,create_time from y_user where floor_id in (select id from y_user where floor_id = {$user_id})";
        }else if($leavel == 3){
            $sql = "select id,tel,token_available_balance,create_time from y_user where floor_id in (select id from y_user where floor_id in (select id from y_user where floor_id = {$user_id}))";
        }
        ...
        if(!empty($userone)){
            ...
            foreach($userone as $key=>$value){
                ...
                $newData[$key]["token_available_balance"] = $value['token_available_balance'];
                $newData[$key]["create_time"] = date("Y-m-d",$value['create_time']);
                ...
                if($value['token_available_balance']>$price){
                    $newData[$key]["account_nature"] = "<span style='color: green;'>有效账号</span>";
                    $newData[$key]["fight_group"] = "<span style='color: green;'>可以拼团</span>";
                    $i=$i+1;
                }else{
                    $newData[$key]["account_nature"] = "<span style='color: red;'>无效账号</span>";
                    $newData[$key]["fight_group"] = "<span style='color: red;'>无法拼团</span>";
                }
            }
...
ON  FIND_IN_SET(b.id, a.users) GROUP BY guid ORDER BY money_total DESC";
    ...
    public function get_user_pin_log($user,$dis_id){
        $userinfo = $this->db->field("id")
            ->table("y_user")
            ->where("username = '{$user}'")
            ->find();
        if(!empty($userinfo['id'])){
            $userpinlog = $this->db->field("*")
                ->table("y_pin_log")
                ->where("dis_id = {$dis_id} AND user_id = {$userinfo['id']}")
                ->find();
            if(!empty($userpinlog['user_sub'])){
               $arrData = explode(",",$userpinlog['user_sub']);
               return  $arrData;
            }else{
                return [];
            }
