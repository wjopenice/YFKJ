...
class offeringModel{
    ...
    public function insert2($username,$data){
        $user = $this->db->field("*")
            ->table("y_user")
            ->where("username = '{$username}'")
            ->find();
        ...
        //$offer['unlock_dyx_num'] = $data['dyx'] * 0.1;
        // 改成全部没有解锁
        $offer['unlock_dyx_num'] = $data['dyx'];
        $offer['lock_dyx_num'] = 0;
        $offer['create_time'] = time();
        $offer['unlock_time'] = 0;
        $bool1 = $this->db->action($this->db->insertSql("offering",$offer));
        $datanum['usdt_num'] = $user['usdt_num'] - $data['input_usdt'];
    ...
    public function insert3($username,$data){
        $user = $this->db->field("*")
            ->table("y_user")
            ->where("username = '{$username}'")
            ->find();
        $user_id = $user['id'];
        $banlance = $this->is_user_dyx($user,(float)$data['input_usdt']);
        $this->db->beginTransaction();
        $offer['user_id'] = $user_id;
        $offer['usdt_num'] = $data['input_usdt'];
        $offer['dyx_num'] = $data['dyx'];
