...
class AdminController extends Controller_Abstract{
    ...
    public function welcomeAction(){
        ...
        //平台总结余
        $data['dyx_total_over'] = $this->db->zssum("user","token_num","num","id NOT IN(8,9,10,11,12,4036,4860,5135,600000,600006)");
        $data['usdt_total_over'] = $this->db->zssum("user","usdt_num","num","id NOT IN(8,9,10,11,12,4036,4860,5135,600000,600006)");
        $oldxyt = $this->db->zssum("pay_wallet_order","xyt_number","num","status = 3");
        $newxyt = $this->db->zssum("user","xyt_num","num","id NOT IN(8,9,10,11,12,4036,4860,5135,600000,600006) AND new_status = 1");
        $data['xyt_total_over'] = $oldxyt + $newxyt;
        $data['btc_total_over'] = $this->db->zssum("user","btc_num","num","id NOT IN(8,9,10,11,12,4036,4860,5135,600000,600006)");
...
ON  FIND_IN_SET(b.id, a.users) GROUP BY guid ORDER BY money_total DESC";
    ...
    public function discountaddAction(){
        if($this->getRequest()->isPost()) {
            ...
            $data['exchange'] = post('exchange');
            $data['lock_exchange_price'] = post('token')."代币 = ".post('usdt')."USDT ,(".date("Y年m月 H:i:s",strtotime(post('dtime'))).")";
            $data['lock85_price'] = post('lock85_price');
            $data['unlock_price'] = post('unlock_price');
            $data['success_price'] = post('success_price');
            $data['system_price'] = 0;
            $data['user_minimum_amount'] = post('user_minimum_amount');
            $data['income'] = "7.06%";
            $data['status'] = 0;
...
//            $title = ["ID","name"];
    ...
    public function auctionAction(){
        include APP_PATH."/application/core/Page.php";
        $page = new \app\core\Page();
        $len = $this->db->zscount("fomo");
        $page->init($len,50);
        $showstr = $page->show();
        $page = $this->db->action("SELECT * FROM y_fomo ORDER BY f_id DESC {$page->limit} ");
        $this->getView()->assign(["arrdata"=>$page,"showstr"=>$showstr]);
    }

    //fomo拍卖活动
    public function auctionaddAction(){
