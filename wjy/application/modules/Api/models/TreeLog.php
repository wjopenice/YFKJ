...
class TreeLogModel extends Model
{
    ...
    public function sendReward($uid, $info)
    {
        ...
        } else
        {
            ...
            {
                // 之前都是用的减法 这里同样用减法
                $dis_money = bcdiv(bcmul($info['money'], 15, 3), 100, 3);
                /** 等级满足 需要设置分销奖励 所以只取85% */
                $log_add['uid'] = $p_user['uid'];
                $log_add['loss_uid'] = 0;
                $log_add['money'] = bcsub($info['money'], $dis_money, 3);
                $disReward = true;
    ...
    {
          $list = $this->where(['uid' => $uid, 'tree_id' => $tree_id])->field('id,uid,from_uid,money,remark,create_time')->page($page, 10)->select();
          /** 获取用户信息 */
          $user_model = new UserModel();
    ...
    /** 获取财富树下的支出和收入 */
    public function incomeAndExpenseByCurrency($uid, $currency_id)
    {

        /** 支出 */
        $expense = $this->where(['uid' => $uid, 'currency_id' => $currency_id])->where('money', '<', 0)->sum('money');

        /** 收入 */
        $income = $this->where(['uid' => $uid, 'currency_id' => $currency_id])->where('money', '>', 0)->sum('money');

        return ['income' => $income, 'expense' => -$expense];
    }

    /** 流水记录 */
    public function treeLogListByCurrency($uid, $currency_id, $page = 1)
    {

        $list = $this->where(['uid' => $uid, 'currency_id' => $currency_id])->field('id,uid,from_uid,money,remark,create_time')->page($page, 10)->select();
        /** 获取用户信息 */
        $user_model = new UserModel();
        foreach ($list as $key => $item)
        {
    ...
    /** 分销流水记录 */
    public function disLogListByCurrency($uid, $currency_id, $level, $page = 1)
    {
        $list = Db::name('dis_log')->where(['uid' => $uid, 'currency_id' => $currency_id, 'level' => $level, 'scene' => 1])->field('id,uid,from_uid,money,remark,create_time')->page($page, 10)->select();
        /** 获取用户信息 */
        $user_model = new UserModel();
        foreach ($list as $key => $item)
        {
