...
class WalletModel extends Model
{
    ...
    public function payMoney($uid, $order_no)
    {
        ...
        if ($pay_order['type'] == 5)
        {
            if (empty($wallet) || $wallet['lock'] < $money)
            {
                return error('money is not enough');
            }
        ...
        if ($pay_order['type'] == 5)
        {
            $wallet_up['lock'] = bcsub($wallet['lock'], $money, 3);
    ...
    /** 增加锁定金额 */
    public function addLuckMoney($uid, $currency, $money, $order_no='', $remark='')
    {
        /** 获取当前币种 */
        $wallet = $this->where(['uid' => $uid, 'currency_id' => $currency])->field(['id', 'total', 'free', 'lock', 'consume'])->find();
        $wallet_up['free'] = bcsub($wallet['free'], $money, 3);
        $wallet_up['lock'] = bcadd($wallet['lock'], $money, 3);
        $wallet_up['update_time'] = time();
        ...
        if ($res)
        {
            ...
            $bill_add['order_no'] = $order_no;
            $bill_add['money'] = $money;
            $bill_add['remark'] = $remark ? $remark : '修改锁定金额';
            $bill_add['create_time'] = time();
            Db::name('bill')->insert($bill_add);
        }
    ...
    public function cashMoney($uid, $currency, $cashInfo)
    {
        ...
        try {
            ...
            $cash_id = Db::name('cash')->insertGetId($cash);
            Db::name('bill')->insert($bill_add);
            $this->where(['id' => $wallet['id']])->update($wallet_up);
            Db::commit();
            return ['result' => 'success', 'order_no' => $order_no, 'cash_id' => $cash_id];

        } catch (\Exception $e) {
            Db::rollback();
            return error('cash fail');
        }
    }
    ...
    /** 获取某币种余额 */
    static function getUserWalletByCurrency($uid, $currency_id)
    {

        $free = self::where(['uid' => $uid, 'currency_id' => $currency_id])->value('free');
        return $free;
    }
    ...
    public function cashOrderno($i = 0)
    {
        ...
        $order_no = 'WC'.$order_no;
        $res = Db::name('cash')->where(['order_no' => $order_no])->find();
        if (empty($res))
        {
            return $order_no;
        }
    ...
    /** 充值记录 */
    static function getRechargeLog($where)
    {
    ...
    /** 结算 */
    static function settlement($count, $currency_id)
    {
        ...
        try {
            ...
            Db::name('bill')->insert($bill_add);
            self::where(['id' => $wallet['id']])->update($wallet_up);
